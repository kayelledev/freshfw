class AddIndexToJoinedTables < ActiveRecord::Migration
  ...
  def self.down
    drop_index :shoppe_suppliers_zones, [:supplier_id, :zone_id], :unique => true
    drop_index :shoppe_cities_zones, [:city_id, :zone_id], :unique => true
    drop_index :shoppe_last_mile_companies_zones, [:last_mile_company_id, :zone_id], :unique => true, :name => 'last_mile_company_zone_index'
    drop_index :shoppe_freight_companies_zones, [:freight_company_id, :zone_id], :unique => true, :name => 'freight_company_zone_index'
  end
end
