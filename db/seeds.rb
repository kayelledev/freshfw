...
city1 = Shoppe::City.where(name: "Monreal", country_id: country.id).first_or_create
city1.zones << zoneA
city2 = Shoppe::City.where(name: "Toronto", country_id: country.id).first_or_create
city2.zones << zoneA
# supplier generation
supplier1 = Shoppe::Supplier.where(name: "Zuo Modern", warehouse: "Monreal", website: "zuomod").first_or_create
supplier1.zones << zoneA
supplier1_zone = Shoppe::SuppliersZone.where(supplier_id: supplier1.id, zone_id: zoneA.id).first
# freight companies
...
# freight routes
route1 = Shoppe::FreightRoute.where(trevel_days: 4, freight_company_id: fcompany1.id, zone_id: zoneB.id, suppliers_zone_id: supplier1_zone).first_or_create
